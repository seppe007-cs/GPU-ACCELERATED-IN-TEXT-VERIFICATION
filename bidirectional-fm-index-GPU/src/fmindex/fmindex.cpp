...
void FMIndex::verifyExactPartialMatchInText(const FMOcc& startMatch,
                                            ...
                                            Occurrences& occ, 
                                            Counters& counters, GPUBuffer& buffer,length_t minED,
    ...
    inTextVerification(partialStarts, maxED, minED, occ, counters,buffer, pattern,
                       beginInPattern == 0);
}
...
void FMIndex::inTextVerification(const vector<length_t>& startPos,
    ...
    length_t nZeros = (fixedStartPos) ? maxED : 3 * maxED ;

    // populate the GPU buffer
    for (const auto& start : startPos) {
        length_t maxEnd = textLength - 1;
        length_t hEnd = min<length_t>(maxEnd, start + pattern.size() + nZeros);

        buffer.starts.emplace_back(start);
        buffer.ends.emplace_back(hEnd);
        buffer.fixed.emplace_back(fixedStartPos);
        buffer.strands.emplace_back(strand);
    }
}
