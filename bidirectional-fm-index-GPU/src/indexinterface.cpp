...
void IndexInterface::goToInTextVerificationEdit(
    ...
    Occurrences& occ, Counters& counters, GPUBuffer& buffer, const Substring& pattern,
    ...
    inTextVerification(pos, s.getMaxED(), s.getMinED(), occ, counters, buffer, pattern,
                       st == 0);
...
void IndexInterface::recApproxMatchEdit(
    ...
    const vector<Substring>& parts, Counters& counters, GPUBuffer& buffer, const int& idx,
    ...
    if (!descendants.empty()) {
        ...
        for (length_t i = 0;
            ...
            if (branchAndBound(
                    bpED, cluster, descendants[i], s, idx, parts, occ, counters, buffer,
                    initOther, descOther,
    ...
    while (!stack.empty()) {
        ...
        if (branchAndBound(bpED, cluster, currentNode, s, idx, parts, occ,
                           counters,buffer, initOther, descOther)) {
        ...
        if (currentNode.getRanges().width() <= inTextSwitchPoint && !idxZero) {
            // crossing-over
            goToInTextVerificationEdit(currentNode, s, parts, occ, counters,buffer,
...
bool IndexInterface::branchAndBound(
    ...
    Occurrences& occ, Counters& counters, GPUBuffer& buffer, const vector<uint16_t>& initOther,
    ...
    if (bpED->inFinalColumn(row)) {
        ...
        if (!validED || bpED->onlyVerticalGapsLeft(row)) {
            ...
            goDeeper(cluster, idx + 1, s, parts, occ, counters,buffer, descOther,
...
void IndexInterface::goDeeper(MatrixMetaInfo& cluster, const length_t& nIdx,
    ...
    if (isEdge) {
        ...
        if (nIdx == parts.size()) {
            ...
            if (match.isValid() && match.getDistance() >= lowerBound) {
                ...
                match.reverseMatchedStr();
#endif // RUN_LENGTH_COMPRESSION
                recApproxMatchEdit(s, match, occ, parts, counters,buffer, nIdx, {}, {},
    ...
    if (switchAfter) {
        ...
        recApproxMatchEdit(s, newMatch, occ, parts, counters,buffer, nIdx, descendants,
...
void IndexInterface::recApproxMatchEditEntry(
    ...
    if (startMatch.getRanges().width() > getSwitchPoint()) {
        ...
        recApproxMatchEdit(search, startMatch, occ, parts, counters,buffer, idx);
        return;
    }
    ...
    verifyExactPartialMatchInText(
        ...
        search.getMaxED(), occ, counters,buffer, search.getMinED(),
