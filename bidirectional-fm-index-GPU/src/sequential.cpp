...
void doBench(vector<ReadBundle>& reads, IndexInterface& mapper,
    ...
    for (unsigned int i = 0; i < reads.size(); i += 1) {
        ...
        matchesPerRead.emplace_back(std::move(matches));
    }
...
#ifndef RUN_LENGTH_COMPRESSION
    FMIndex index(params.base, params.inTextVerificationPoint, params.noCIGAR,
                  params.sparsenessFactor, true, params.kmerSize);
#else
    BMove index(params.base, true, params.noCIGAR, params.kmerSize);
#endif
