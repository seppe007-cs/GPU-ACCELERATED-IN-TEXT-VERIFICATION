...
void SearchStrategy::matchWithSearches(const string& seq, const length_t k,
                                       Counters& counters, Occurrences& occs, GPUBuffer& buffer,
    ...
    for (uint16_t i = 0; i < numParts; i++) {
        ...
        if (width != 0 && width <= index.getSwitchPoint()) {
            ...
            (this->*inTextVerificationPtr)(startMatch, part.begin(), k, parts,
                                           occs, counters,buffer, minDistance);
        }
    }
#endif
    ...
    for (const Search& s : searches) {
        doRecSearch(s, parts, occs, exactMatchRanges, counters, buffer);
    }
}

void SearchStrategy::matchApproxAllMap(ReadBundle& bundle, length_t maxED,
                                       Counters& counters,
                                       vector<FMOcc>& result, GPUBuffer& buffer) {
    ...
    const std::string& read = bundle.getRead();
    buffer.reads.reserve(buffer.reads.size() + read.size());
    buffer.reads.insert(buffer.reads.end(), read.begin(), read.end());
    buffer.offsets.push_back(buffer.offsets.back() + read.size());

    matchWithSearches(bundle.getRead(), maxED, counters, occ, buffer);

    buffer.numIntextperRead.push_back(buffer.starts.size());

    // set the index in reverse complement mode with the correct sequence
    index.setIndexInMode(REVERSE_C_STRAND);

    // map with the searches
    const std::string& readRC = bundle.getRevComp();
    buffer.reads.reserve(buffer.reads.size() + readRC.size());
    buffer.reads.insert(buffer.reads.end(), readRC.begin(), readRC.end());
    buffer.offsets.push_back(buffer.offsets.back() + readRC.size());

    matchWithSearches(bundle.getRevComp(), maxED, counters, occ, buffer);

    buffer.numIntextperRead.push_back(buffer.starts.size());
    
    result = occ.getFMOccurrencesMutable();
}
...
void SearchStrategy::doRecSearch(const Search& s, vector<Substring>& parts,
                                 ...
                                 Counters& counters, GPUBuffer& buffer) const {
    ...
    if (s.getUpperBound(0) > 0) {
        ...
        (this->*startIdxPtr)(s, startMatch, occ, parts, counters, buffer, 0);
        return;
    }
    ...
    if (startRange.width() > index.getSwitchPoint()) {
        ...
        (this->*startIdxPtr)(s, startMatch, occ, parts, counters, buffer, partInSearch);
    }
}
...
#ifndef RLC
void SearchStrategy::postProcessingAfterGPU(ReadBundle& readBundle, std::vector<FMOcc>& inIndexMatches, const GPUBuffer& buffer, const std::vector<uint32_t> buffelements, 
		                                    const length_t maxED, Counters& counters, std::vector<TextOcc>& results, const size_t read_index){
    
    // in this case no paired end, so we set the satus fixed to first in pair
    const PairStatus status = FIRST_IN_PAIR;

    Occurrences occ;
    std::vector<uint32_t> Zerobuff;
    
    // number of in-text candidates for this read
    const size_t numBuffElements = buffelements[read_index+1]-buffelements[read_index];

    // add all in index occurrences to Occurrences structure
    occ.addFMOccs(inIndexMatches);

    // make pointers to matrices
    IBitParallelED* matrixForward;
    IBitParallelED* matrixReverse;

    // set the read sequences in the matrices
    index.getFullReadMatrix(FORWARD_STRAND,status).setSequence(readBundle.getRead());
    index.getFullReadMatrix(REVERSE_C_STRAND,status).setSequence(readBundle.getRevComp());
    index.getFullReadMatrix128(FORWARD_STRAND,status).setSequence(readBundle.getRead());
    index.getFullReadMatrix128(REVERSE_C_STRAND,status).setSequence(readBundle.getRevComp());  

    // iterate over all in index occurrences for this read
    for(size_t i=0; i<numBuffElements;i++){

        // only verify those with ED <= max aloud ED
        if(buffer.results[buffelements[read_index]+i]<= maxED){

            bool matrix128 = ((buffer.fixed[buffelements[read_index]+i]<1 && maxED>=7) || maxED>10);
            if (matrix128){
                matrixForward = &index.getFullReadMatrix128(FORWARD_STRAND,status);
                matrixReverse = &index.getFullReadMatrix128(REVERSE_C_STRAND,status);
            }else{
                matrixForward = &index.getFullReadMatrix(FORWARD_STRAND,status);
                matrixReverse = &index.getFullReadMatrix(REVERSE_C_STRAND,status);
            }

            // initialize the top row for the matrix
            length_t nZeros = (buffer.fixed[buffelements[read_index]+i]) ? 1 : 2 * maxED + 1;
            Zerobuff.resize(nZeros);

            // get the reference substring
            std::vector<Substring> ref = {Substring(index.getText(),buffer.starts[buffelements[read_index]+i],
                                    buffer.ends[buffelements[read_index]+i])};

            // initialize the correct matrix
            if(buffer.strands[buffelements[read_index]+i] == FORWARD_STRAND){
                matrixForward->initializeMatrix(maxED, Zerobuff);
            } else {
                matrixReverse->initializeMatrix(maxED, Zerobuff);
            }

            // do complete intextverification on CPU
            if (!matrix128) {
                InTextVerificationTaskCPU<uint64_t> task(
                    ref, &index.getFullReadMatrix(buffer.strands[buffelements[read_index]+i], status), maxED, 0, buffer.strands[buffelements[read_index]+i], status, index.getNoCIGAR());
                task.doTask(counters, occ);
            } else {
                InTextVerificationTaskCPU<UInt128> task(
                    ref, &index.getFullReadMatrix128(buffer.strands[buffelements[read_index]+i], status), maxED, 0, buffer.strands[buffelements[read_index]+i], status, index.getNoCIGAR());
                task.doTask(counters, occ);
            }
        }
    }
    
    // get all non-redundant matches mapped to the text
    results = (this->*filterPtr)(occ, maxED, counters, readBundle);

    // generate sam output
    if (generateSAMLines) {
        generateOutputSingleEnd(results, readBundle, counters, maxED);
    }
}
#endif //RLC
