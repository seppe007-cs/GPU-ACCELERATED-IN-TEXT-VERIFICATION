...
void processChunk(vector<ReadBundle>& input, OutputChunk& output,
    ...
    // Buffer for GPU processing
    GPUBuffer buffer(input.size(),strategy->getSwitchPoint(),maxEDOrIdentity);
    
    // stores the total number of in-text candidates per readbundle
    std::vector<uint32_t> buffer_elements;
    buffer_elements.reserve(input.size()+1);
    buffer_elements.push_back(0);

    // stores in-index matches per readbundle
    std::vector<std::vector<FMOcc>> matches_per_read;
    matches_per_read.reserve(input.size());

    // perform the search per readbundle
    for (auto& readBundle : input) {
        std::vector<FMOcc> inIndexMatches;
        inIndexMatches.reserve(1000); // pre-allocate some space
        strategy->matchApprox(readBundle, maxEDOrIdentity,
                              output.getMutableCounters(), inIndexMatches, buffer);
        buffer_elements.push_back(buffer.starts.size());
        matches_per_read.push_back(std::move(inIndexMatches));
    }

    // start gpu in-text verification (filtering)
    GPU_intextVerification(buffer, maxEDOrIdentity);

    // post-processing per readbundle
    for(size_t i=0; i< input.size();i++){
        std::vector<TextOcc> matches;
        strategy->postProcessingAfterGPU(input[i],matches_per_read[i],buffer, buffer_elements,
                                         maxEDOrIdentity,output.getMutableCounters(),matches, i);
    
        output.addSingleEndRecord(std::move(matches));
    }

    matches_per_read.clear();
...
std::unique_ptr<Reader> createReader(const Parameters& params) {
    try {
        ...
        size_t targetChunk = 200000/params.nThreads;
...
int main(int argc, char* argv[]) {
    ...
    // GPU initialization --> set text and char2idx
    cudamalloc_and_memcpy_text(strategy->getText());
    cudamalloc_and_memcpy_char2idx();
