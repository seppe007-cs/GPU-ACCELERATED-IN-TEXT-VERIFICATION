...
void generatePredToRun(const vector<length_t>& samplesFirst,
    ...
    assert(textLength > 0);
    assert(!samplesFirst.empty());
    assert(!samplesLast.empty());
    assert(samplesFirst.size() == samplesLast.size());

    firstToRun.resize(samplesFirst.size());
    for (length_t i = 0; i < firstToRun.size(); ++i)
        firstToRun[i] = i;
    sort(firstToRun.begin(), firstToRun.end(),
         [&samplesFirst, &textLength](length_t a, length_t b) {
             assert(a < samplesFirst.size());
             assert(b < samplesFirst.size());
    ...
    for (length_t i = 0; i < lastToRun.size(); ++i)
        lastToRun[i] = i;
    sort(lastToRun.begin(), lastToRun.end(),
         [&samplesLast, &textLength](length_t a, length_t b) {
             assert(a < samplesLast.size());
             assert(b < samplesLast.size());
